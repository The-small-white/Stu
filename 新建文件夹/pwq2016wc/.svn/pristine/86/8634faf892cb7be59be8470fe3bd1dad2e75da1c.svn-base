...
{
    protected TrdType news = new TrdType();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (action != "save")
        {
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                condition.ID = _iD;
                news = TableOperate<TrdType>.GetRowData(condition);
        ...
        else
        {
            ...
            string logbrief = "";
            TrdType newChannelNews = new TrdType();
            newChannelNews.ID = 0;
            newChannelNews.AutoForm(this.Page);
            string Title = newChannelNews.TradeType;
            int _iD;
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                _iD = Convert.ToInt32(this.Request["iD"]);
                TableOperate<TrdType>.Update(newChannelNews);
                result.msg = "编辑成功，等待返回列表";
                logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "编辑了为【" + Title + "】的行业";
            }
            else
            {
                ...
                _iD = TableOperate<TrdType>.InsertReturnID(newChannelNews);
                result.msg = "添加成功，等待返回列表";
                logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "添加了为【" + Title + "】的行业";
            }
