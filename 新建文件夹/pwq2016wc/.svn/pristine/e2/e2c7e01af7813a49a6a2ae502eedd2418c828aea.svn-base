/************************************************************
  ...
  Description:    数据表CloudLog对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与CloudLog数据表对应对象
    /// </summary>
    public class CloudLog : IDateBuildTable
    {
        ...
        public CloudLog(int iD, int addID, DateTime addTime, string brief, int exhibitionID)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || brief_initialized || exhibitionID_initialized)
                {
                    return false;
                }
                else
