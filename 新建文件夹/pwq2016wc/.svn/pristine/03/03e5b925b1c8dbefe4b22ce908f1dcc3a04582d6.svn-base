/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/7/26 17:07:46
  Description:    数据表SpeechTitle对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与SpeechTitle数据表对应对象
    /// </summary>
    public class SpeechTitle : IDateBuildTable
    {
        ...
        private bool key_Word_initialized = false;
        ...
        public SpeechTitle(int iD, int exhibitionID, string key_Word, string title, string breif, int addID, DateTime addTime)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "Key_Word"))
            {
                if (dr["Key_Word"] != null && dr["Key_Word"] != DBNull.Value)
                {
                    this.Key_Word = Convert.ToString(dr["Key_Word"]);
                }
            }
        ...
        public string Key_Word
        {
            get
            {
                return this.m_key_Word;
            }
            set
            {
                key_Word_initialized = true;
                this.m_key_Word = value;
            }
        }
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || exhibitionID_initialized || key_Word_initialized || title_initialized || breif_initialized || addID_initialized || addTime_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            if (key_Word_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Key_Word", SqlDbType.NVarChar);
                n_Parameter.Value = this.Key_Word;
                n_Parameter.SourceColumn = "Key_Word";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.key_Word_initialized)
                {
                    contentText += ", [Key_Word]=@" + headStr + "Key_Word ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.key_Word_initialized)
                {
                    conditionStr += " AND [Key_Word]=@" + headStr + "Key_Word ";
                }
        ...
        public void AllInitialized()
        {
            ...
            this.key_Word_initialized = true;
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["key_Word"] != null)
            {
                if (this.key_Word_initialized)
                {
                    if(page.Request["key_Word"] != "")
                    {
                        this.Key_Word = Convert.ToString(page.Request["key_Word"]);
                    }
                    else
                    {
                        this.key_Word_initialized = false;
                    }
                }
                else
                {
