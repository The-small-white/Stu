<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FaceTime.aspx.cs" Inherits="Plusbe2019_ManageUserinfo_Manage" %>
                            ...
                            <table class="table table-bordered table-hover">
                                <thead class="text-c">
                                 ...
                                 <asp:Repeater ID="Repeater" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((UserFaceTime)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((UserFaceTime)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                   
                                            <td><%#((UserFaceTime)Container.DataItem).UserID%></td>
                                                <td><%#((UserFaceTime)Container.DataItem).AddTime%></td>
                                               <td><%#((UserFaceTime)Container.DataItem).LastTime%></td>
                                                <td><%#Image(((UserFaceTime)Container.DataItem).HeadPic)%></td>
                                             <td><%#((UserFaceTime)Container.DataItem).ZQID%></td>
                                                <td><%#((UserFaceTime)Container.DataItem).Quality%></td>
                                               
                                          
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((Face_Userinfo_View)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((Face_Userinfo_View)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>                     
