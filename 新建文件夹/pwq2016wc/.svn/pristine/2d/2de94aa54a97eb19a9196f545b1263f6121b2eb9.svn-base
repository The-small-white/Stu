/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/27 15:15:59
  Description:    数据表View_Question对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与View_Question数据表对应对象
    /// </summary>
    public class View_Question : IDateBuildTable
    {
        ...
        private bool answer_initialized = false;
        ...
        private string m_trueList;
        private bool trueList_initialized = false;
        ...
        public View_Question(string name, int iD, string title, int typeID, string files, string answer, int exhibitionID, long orderID, DateTime addTime, int addID, string brief, string fileSize, string trueList)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "TrueList"))
            {
                if (dr["TrueList"] != null && dr["TrueList"] != DBNull.Value)
                {
                    this.TrueList = Convert.ToString(dr["TrueList"]);
                }
            }
        ...
        public string TrueList
        {
            get
            {
                return this.m_trueList;
            }
            set
            {
                trueList_initialized = true;
                this.m_trueList = value;
            }
        }
        ...
        public bool IsNull
        {
            get
            {
                if (name_initialized || iD_initialized || title_initialized || typeID_initialized || files_initialized || answer_initialized || exhibitionID_initialized || orderID_initialized || addTime_initialized || addID_initialized || brief_initialized || fileSize_initialized || trueList_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Answer", SqlDbType.NVarChar);
                n_Parameter.Value = this.Answer;
                n_Parameter.SourceColumn = "Answer";
                parametersList.Add(n_Parameter);
            }
            ...
            if (trueList_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "TrueList", SqlDbType.NVarChar);
                n_Parameter.Value = this.TrueList;
                n_Parameter.SourceColumn = "TrueList";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.answer_initialized)
                {
                    contentText += ", [Answer]=@" + headStr + "Answer ";
                }
                ...
                if (this.trueList_initialized)
                {
                    contentText += ", [TrueList]=@" + headStr + "TrueList ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.answer_initialized)
                {
                    conditionStr += " AND [Answer]=@" + headStr + "Answer ";
                }
                ...
                if (this.trueList_initialized)
                {
                    conditionStr += " AND [TrueList]=@" + headStr + "TrueList ";
                }
        ...
        {
            string tableName = "View_Question";
            return tableName;
        }
        ...
        public void AllInitialized()
        {
            ...
            this.trueList_initialized = true;
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["answer"] != null)
            {
                if (this.answer_initialized)
                {
                    if(page.Request["answer"] != "")
                    {
                        this.Answer = Convert.ToString(page.Request["answer"]);
                    }
                    else
                    {
                        this.answer_initialized = false;
                    }
                }
                else
                {
            ...
            if(page.Request["trueList"] != null)
            {
                if (this.trueList_initialized)
                {
                    if(page.Request["trueList"] != "")
                    {
                        this.TrueList = Convert.ToString(page.Request["trueList"]);
                    }
                    else
                    {
                        this.trueList_initialized = false;
                    }
                }
                else
                {
