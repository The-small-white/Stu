             ...
             <div class="ibox-content">
                        <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                            <div class="form-group">
                                <input type="text" placeholder="请输入展区名"  name="name" id="name"  runat="server" class="form-control">
                            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((Area)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((Area)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                    <td><%# ((Area)Container.DataItem).ID%></td>
                                    <td><%# ((Area)Container.DataItem).AreaName%></td>
                                    <td><%# ((Area)Container.DataItem).AddTime%></td>
                                     <td>
                                        <span   data-id="<%#((Area)Container.DataItem).ID%>" class=" up glyphicon glyphicon-arrow-up" aria-hidden="true"></span>
                                        <span  data-id="<%#((Area)Container.DataItem).ID%>" class=" down glyphicon glyphicon-arrow-down" aria-hidden="true"></span>

                                                  </td>
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((Area)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((Area)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
