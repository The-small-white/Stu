/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/21 9:20:08
  Description:    数据表ReserveDate对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与ReserveDate数据表对应对象
    /// </summary>
    public class ReserveDate : IDateBuildTable
    {
        ...
        public ReserveDate(int iD, int addID, DateTime addTime, int exhibitionID, string date, string brief, int count, long orderID)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || exhibitionID_initialized || date_initialized || brief_initialized || count_initialized || orderID_initialized)
                {
                    return false;
                }
                else
