             ...
             <div class="ibox-content">
                        <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                            <div class="form-group">
                                <input type="text" placeholder="请输入关键词"  name="name" id="name"  runat="server" class="form-control">
                            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((SpeechTitle)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((SpeechTitle)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                    <td><%# ((SpeechTitle)Container.DataItem).Title%></td>
                                    <td><%# ((SpeechTitle)Container.DataItem).Key_Word%></td>
                                  
                                    <td class="text-c">
                                        <a  href="../SpeechCmd/Manage.aspx?SpeechID=<%#((SpeechTitle)Container.DataItem).ID%>"  class="btn btn-sm btn-success" ><i class="fa fa-bars"></i> 指令管理</a>
                                            <a href="#"  class="btn btn-sm btn-info editmySelf"  data-id="<%#((SpeechTitle)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑</a>
                                             <a href="#" class="btn btn-danger btn-sm deleteSelf"  data-id="<%#((SpeechTitle)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除</a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
