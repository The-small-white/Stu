...
public partial class Admin_AreaManageDevice_Delete : AdminPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                AddLog(iD + "");
                if (TableOperate<Device>.Delete(iD) > 0)
                {
                    result.isOk = true;
            ...
            else if (!string.IsNullOrEmpty(this.Request["checkshop"]))
            {
                ...
                AddLog(checkshop);
                int id = TableOperate<Device>.MultiDelete(checkshop);
                if (id > 0)
                {
                    result.isOk = true;
    ...
    protected void AddLog(string ids)
    {
        ...
        List<Device> list = TableOperate<Device>.Select(value, condition);
        if (list.Count > 0)
        {
            string deltitle = "";
            ...
            if (deltitle != "")
            {
                string logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "删除了【" + deltitle + "】的设备";
