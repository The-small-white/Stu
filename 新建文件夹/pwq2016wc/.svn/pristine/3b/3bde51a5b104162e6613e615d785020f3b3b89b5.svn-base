...
<head>
    ...
    <link rel="shortcut icon" href="<%=SysConfig.Hui %>favicon.ico"> <link href="<%=SysConfig.Hui %>css/bootstrap.min.css?v=3.3.6" rel="stylesheet">
...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
             <div class="ibox-content">
                        <form role="form" action="Manage.aspx" method="get" class="form-inline">
                            <div class="form-group">
                                <input type="text" placeholder="请输入表名"  class="form-control">
                            </div>
                       ...
                       <form id="theform" name="theform"  method="post">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                ...
                                <tr>
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((TableManage)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                    <td><%# ((TableManage)Container.DataItem).ID%></td>
                                    <td><%# ((TableManage)Container.DataItem).Name%></td>
                                    <td><%# ((TableManage)Container.DataItem).Breif%></td>
                                    <td><%# ((TableManage)Container.DataItem).AddTime%></td>
                                    <td><%# ((TableManage)Container.DataItem).AddName%></td>
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((TableManage)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((TableManage)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
    ...
    <script>
        ...
        $('.editmySelf').click(function () {
            ...
            layer.open({
                ...
                fixed: false, //不固定
                maxmin: true,
                content: 'edit.aspx?id=' + $this.data('id')
            });
        });
