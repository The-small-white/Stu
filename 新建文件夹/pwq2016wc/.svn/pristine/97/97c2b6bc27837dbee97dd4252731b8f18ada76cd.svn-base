...
public partial class Admin_ManageTableManage_Delete : AdminPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                if (TableOperate<CloudLog>.Delete(iD) > 0)
                {
                    result.isOk = true;
            ...
            else if (!string.IsNullOrEmpty(this.Request["checkshop"]))
            {
                ...
                int id = TableOperate<CloudLog>.MultiDelete(checkshop);
                if (id > 0)
                {
                    result.isOk = true;
            ...
            else if (!string.IsNullOrEmpty(this.Request["all"]))
            {
                ...
                int id = TableOperate<CloudLog>.Delete(condition);
                if (id > 0)
                {
                    result.isOk = true;
    ...
    protected void AddLog(string ids)
    {
        ...
        CloudLog condition = new CloudLog();
        CloudLog value = new CloudLog();
        condition.AddConditon(" and id in(" + ids + ")");
        List<CloudLog> list = TableOperate<CloudLog>.Select(value, condition);
        if (list.Count > 0)
        {
            string deltitle = "";
