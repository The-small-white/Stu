...
{
    protected Userinfo news = new Userinfo();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (action != "save")
        {
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                condition.ID = _iD;
                news = TableOperate<Userinfo>.GetRowData(condition);
                iD.Value = Convert.ToString(news.ID);
                Gender.Value = Convert.ToString(news.Gender);
        ...
        else
        {
            ...
            string logbrief = "";
            Userinfo newChannelNews = new Userinfo();
            newChannelNews.ID = 0;
            ...
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                TableOperate<Userinfo>.Update(newChannelNews);
                result.msg = "编辑成功，等待返回列表";
                logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "编辑了为【" + Title + "】的用户";
            }
            else
            {
                ...
                _iD = TableOperate<Userinfo>.InsertReturnID(newChannelNews);
                result.msg = "添加成功，等待返回列表";
                logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "添加了为【" + Title + "】的用户";
            }
