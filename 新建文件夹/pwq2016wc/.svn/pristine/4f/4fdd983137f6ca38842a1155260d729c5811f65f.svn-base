/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/23 14:25:33
  Description:    数据表ChannelNews对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与ChannelNews数据表对应对象
    /// </summary>
    public class ChannelNews : IDateBuildTable
    {
        ...
        public ChannelNews(int iD, int addID, DateTime addTime, int channelID, string brief, int typeID, string files, string files1, int exhibitionID, string title, long orderID, string fileSize)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || channelID_initialized || brief_initialized || typeID_initialized || files_initialized || files1_initialized || exhibitionID_initialized || title_initialized || orderID_initialized || fileSize_initialized)
                {
                    return false;
                }
                else
