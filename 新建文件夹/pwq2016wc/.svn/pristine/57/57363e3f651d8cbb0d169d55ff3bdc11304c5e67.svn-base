                    ...
                    <div class="ibox-content">
                       ...
                       <form id="theform" name="theform"  method="post">
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    <th width="8%"  class="text-c">
                                         <input type="checkbox" class="i-checks"  id="allcheck">
                                    </th>
                                    <th  class="text-c">序列</th>
                                    <th  class="text-c">ID</th>
                                   
                                    <th>日志</th>
                                    <th  class="text-c">操作时间</th>
                                    
                                    <th  class="text-c">编辑</th>
                                </tr>
                            </thead>
                            <tbody>
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((CloudLog)Container.DataItem).ID%>">
                                    <td  class="text-c"> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((CloudLog)Container.DataItem).ID%>">
                                    </td>
                                    <td  class="text-c"><%#Container.ItemIndex+1%></td>
                                    <td  class="text-c"><%# ((CloudLog)Container.DataItem).ID%></td>
                                    <td><%# ((CloudLog)Container.DataItem).Brief%></td>
                            
                                    <td  class="text-c"><%# ((CloudLog)Container.DataItem).AddTime%></td>
                                   
                                    <td class="text-c">

                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((CloudLog)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
