/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/21 11:22:38
  Description:    数据表View_Reserve对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与View_Reserve数据表对应对象
    /// </summary>
    public class View_Reserve : IDateBuildTable
    {
        ...
        private bool date_initialized = false;
        ...
        public View_Reserve(int iD, int addID, DateTime addTime, int exhibitionID, DateTime reserveTime, int dateType, string reserveName, string reservePhone, int reserveCount, int userInfoID, string openID, int states, string brief, string date)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || exhibitionID_initialized || reserveTime_initialized || dateType_initialized || reserveName_initialized || reservePhone_initialized || reserveCount_initialized || userInfoID_initialized || openID_initialized || states_initialized || brief_initialized || date_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            if (date_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Date", SqlDbType.NVarChar);
                n_Parameter.Value = this.Date;
                n_Parameter.SourceColumn = "Date";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.date_initialized)
                {
                    contentText += ", [Date]=@" + headStr + "Date ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.date_initialized)
                {
                    conditionStr += " AND [Date]=@" + headStr + "Date ";
                }
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["date"] != null)
            {
                if (this.date_initialized)
                {
                    if(page.Request["date"] != "")
                    {
                        this.Date = Convert.ToString(page.Request["date"]);
                    }
                    else
                    {
                        this.date_initialized = false;
                    }
                }
                else
                {
