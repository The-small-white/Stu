...
public partial class Admin_ManageTableManageReserveDate_Delete : AdminPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                AddLog(iD + "");
                if (TableOperate<ReserveDate>.Delete(iD) > 0)
                {
                    result.isOk = true;
            ...
            else if (!string.IsNullOrEmpty(this.Request["checkshop"]))
            {
                ...
                AddLog(checkshop);
                int id = TableOperate<ReserveDate>.MultiDelete(checkshop);
                if (id > 0)
                {
                    result.isOk = true;
