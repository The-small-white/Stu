...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
             <div class="ibox-content">
                        ...
                        <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                            <button class="btn btn-white " type="button" id="back"><i class="fa fa-arrow-left"></i> 返回</button>
                            <div class="form-group">
                                <input type="text" placeholder="请输入标题"  name="name" id="name"  runat="server" class="form-control">
                            </div>
                            <div class="form-group">
                                  <select class="chosen-select" name="pcid"    id="pcid" style="width:180px;">
                                     <option value="0">所有</option>
                                     <asp:Repeater ID="repeater" runat="server" DataSource="<%#CloudSQL.GetMyPCList() %>">
                                       <ItemTemplate>
                                     <option  value="<%#((Pclist)Container.DataItem).ID %>" <%# SysConfig.Selected(((Pclist)Container.DataItem).ID+"",PCID+"") %>>
                                         <%#((Pclist)Container.DataItem).Title %>
                                     </option>
                                      </ItemTemplate>
                                       </asp:Repeater>
                                </select>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead class="text-c">
                                <tr>
                                    ...
                                    <th style="display:<%=AdminMethod.CheckManage("zhiyuan")?"":"none"%>">分享状态</th>
                                    <th>排序</th>
                                     <th>更新时间</th>
                                    <th>编辑</th>
                                </tr>
                            ...
                            <tbody class="text-c">
                                ...
                                <tr id="tr_<%# ((View_News)Container.DataItem).ID%>">
                                    ...
                                    <td><%# ((View_News)Container.DataItem).ModeName%></td>
                                    <td><%# ((View_News)Container.DataItem).ChannelName%></td>
                                    <td><%# ((View_News)Container.DataItem).TagName%></td>
                                    <td><%# GetStates(((View_News)Container.DataItem).State)%></td>
                                   
                                    <td style="display:<%=AdminMethod.CheckManage("zhiyuan")?"":"none"%>"><%# GetShare(((View_News)Container.DataItem).IsShare)%></td>
                                    <td>
                                        <span   data-id="<%#((View_News)Container.DataItem).ID%>" class=" top glyphicon fa fa-eject" aria-hidden="true"></span>
                                        <span   data-id="<%#((View_News)Container.DataItem).ID%>" class=" up glyphicon glyphicon-arrow-up" aria-hidden="true"></span>
                                        <span  data-id="<%#((View_News)Container.DataItem).ID%>" class=" down glyphicon glyphicon-arrow-down" aria-hidden="true"></span>

                                    </td>
                                     <td><%# ((View_News)Container.DataItem).AddTime.ToString()%></td>
                                    <td class="text-c">
                                        
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((View_News)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-sm btn-default sharemySelf" style="display:<%=AdminMethod.CheckManage("zhiyuan")?"":"none"%>" data-id="<%#((View_News)Container.DataItem).ID%>"><i class="fa fa-share-alt"></i> 分享 </a>
                                            <a href="#" class="btn btn-sm btn-primary download"   data-id="<%#((View_News)Container.DataItem).ID%>"><i class="fa fa-cloud-download"></i>下载</a>
                                            <a href="#" class="btn btn-sm btn-success spmySelf" data-id="<%#((View_News)Container.DataItem).ID%>"><i class="fa fa-check"></i> 展示 </a>
                                            
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((View_News)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
                             ...
                             <tfoot> <tr><td colspan="12" class="footable-visible"> <%#this.GetPagingHtml() %></td> </tr></tfoot>
                        </table>
                        </form>
    ...
    <script>
        $(document).ready(function ()
        {
            $('#back').click(function ()
            {
                location.href = '../PCManage/Manage.aspx';
            });
        ...
        $('.editmySelf').click(function () {
            ...
            var url = 'edit.aspx?iD=' + $this.data('id') + '&pcid=<%=PCID%>';
            if (ids == 0) {
                url = 'edit.aspx?pcid=<%=PCID%>';
            }
                ...
                success: function (result) {
                    if (result != "") {
                        ...
                        if ("up" == action) {
                            ...
                            location.href = 'manage.aspx?pcid=<%=PCID%>';
                        }
                    }
                }
            });
