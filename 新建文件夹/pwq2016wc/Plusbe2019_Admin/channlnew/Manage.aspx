...
<body class="gray-bg">
                            ...
                            <div class="form-group">
                                  <select class="chosen-select" name="pcid"    id="pcid" style="width:180px;">
                                     <asp:Repeater ID="repeater" runat="server" DataSource="<%#m_ExhibitionList %>">
                                       <ItemTemplate>
                                       <option  value="<%#((ModeChannel)Container.DataItem).ID %>" <%# SysConfig.Selected(((ModeChannel)Container.DataItem).ID+"",PCID+"") %> <%# ((ModeChannel)Container.DataItem).Child?"disabled":""%>>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((ChannelNews)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((ChannelNews)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>  
                                    <td><%# ((ChannelNews)Container.DataItem).Title%></td>
                                   
                                    <td>
                                        <span   data-id="<%#((ChannelNews)Container.DataItem).ID%>" class=" up glyphicon glyphicon-arrow-up" aria-hidden="true"></span>
                                        <span  data-id="<%#((ChannelNews)Container.DataItem).ID%>" class=" down glyphicon glyphicon-arrow-down" aria-hidden="true"></span>

                                    </td>
                                    <td class="text-c">
                                        

                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((ChannelNews)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((ChannelNews)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
    ...
    <script>
        $(document).ready(function ()
        {
            $('#back').click(function ()
            {
                location.href = '../AdminChannel/Manage.aspx';
            });
