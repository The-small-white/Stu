...
<body class="gray-bg">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((SpeechSend)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((SpeechSend)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                  
                                    <td><%# ((SpeechSend)Container.DataItem).Title%></td>
                                    <td><%# ((SpeechSend)Container.DataItem).IP%>/<%# ((SpeechSend)Container.DataItem).Port%></td>
                                    <td><%# (SysConfig.CharTypeState)((SpeechSend)Container.DataItem).CharType%></td>
                                    <td><%# (SysConfig.ProtocolState)((SpeechSend)Container.DataItem).Protocol%></td>
                                    <td><%# ((SpeechSend)Container.DataItem).SendCode%></td>
                                
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((SpeechSend)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((SpeechSend)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
    ...
    <script>
        ...
        {
            $('#back').click(function () {
                location.href = '../SpeechManage/Manage.aspx';
            });
            $('#pcid').chosen({
        ...
        $('.editmySelf').click(function () {
            ...
            var url = 'edit.aspx?iD=' + $this.data('id') + '&SpeechID=<%=SpeechID%>';
            if (ids == 0) {
                url = 'edit.aspx?SpeechID=<%=SpeechID%>';
            }
