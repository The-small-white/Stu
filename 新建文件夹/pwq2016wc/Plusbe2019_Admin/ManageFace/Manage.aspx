...
<body class="gray-bg">
                            ...
                            <table class="table table-bordered table-hover">
                                <thead class="text-c">
                                 ...
                                 <asp:Repeater ID="Repeater" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((Face)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((Face)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                   
                                            
                                               
                                                <td><%#Image(((Face)Container.DataItem).HeadImage)%></td>
                                             
                                                <td><%#((Face)Container.DataItem).AddTime%></td>
                                               
                                          
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((Face)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((Face)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                         <a href="#" class="btn btn-danger btn-sm shenheSelf" data-id="<%#((Face)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 审核 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>                     
    ...
    <script>
        ...
        $('.shenheSelf').click(function () {
            var $this = $(this);
            layer.confirm('确定要审批该信息？', {
                btn: ['确定', '取消'] //按钮
            }, function () {
                    $.post('ShenHe.aspx', { id: $this.data('id') }, function (result) {
                    if (result.isOk) {
