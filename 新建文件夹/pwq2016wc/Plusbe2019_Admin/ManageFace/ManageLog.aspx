<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ManageLog.aspx.cs" Inherits="Plusbe2019_ManageUserinfo_Manage" %>
                            ...
                            <table class="table table-bordered table-hover">
                                <thead class="text-c">
                                 ...
                                 <asp:Repeater ID="Repeater" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((FaceLog)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((FaceLog)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                   
                                            <td><%#((FaceLog)Container.DataItem).Msg%></td>
                                                
                                                <td><%#((FaceLog)Container.DataItem).AddTime%></td>
                                               
                                          
                                    <td class="text-c">
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((FaceLog)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>                     
    ...
    <script>
        ...
        $('.deleteSelf').click(function () {
            ...
            layer.confirm('确定要删除该信息？', {
                ...
                {
                    $.post('DeleteLog.aspx', { id: $this.data('id') }, function (result) {
                    if (result.isOk) {
        ...
        $('#deall').click(function () {
            ...
            layer.confirm('确定要删除全部信息？', {
                btn: ['确定', '取消'] //按钮
                    ...
                    $.post('DeleteLog.aspx', { all: 1 }, function (result) {
                    if (result.isOk) {
                        layer.msg('删除成功', { icon: 1, time: 1000 });
                        location.reload();
                    } else {
            ...
            if ($("input[type='checkbox']:checked").length == 0) {
                ...
                layer.confirm('确定要删除该信息？', {
                    btn: ['确定', '取消'] //按钮
                        ...
                        $.post('DeleteLog.aspx', arr, function (result) {
                            if (result.isOk) {
                                layer.msg('删除成功', { icon: 1, time: 1000 });
