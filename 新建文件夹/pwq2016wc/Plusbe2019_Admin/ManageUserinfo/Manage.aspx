<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Manage.aspx.cs" Inherits="Plusbe2019_ManageUserinfo_Manage" %>
        ...
        <div class="ibox-content">
            <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                <div class="form-group">
                        <input type="text" placeholder="姓名/昵称" name="name" id="name"  value="<%=Request["name"] %>"  class="form-control">
                        <input type="text" placeholder="联系方式" name="phone" id="phone"  value="<%=Request["phone"] %>"  class="form-control">
                            </div>
            ...
            <div class="col-sm-12">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <button class="btn btn-success" type="button" id="set">标记永久</button>
                         <button class="btn btn-warning"  type="button" id="deall">删除非标记用户</button>
                        <form id="theform" name="theform" method="post">
                            <table class="table table-bordered table-hover">
                                <thead class="text-c">
                                    ...
                                    <tbody class="text-c">
                                 <asp:Repeater ID="Repeater" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((Userinfo_View)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((Userinfo_View)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                   
                                            <td><%#((Userinfo_View)Container.DataItem).Name%></td>
                                            <td><%#((Userinfo_View)Container.DataItem).State==1?"保存":"临时"%></td>
                                                <td><%#((Userinfo_View)Container.DataItem).Name%></td>
                                               
                                                <td><%#((Userinfo_View)Container.DataItem).Phone%></td>
                                                <td><%#((Userinfo_View)Container.DataItem).NickName%></td>
                                               
                                                <td><%#Image(((Userinfo_View)Container.DataItem).HeadImage)%></td>
                                           <td>
                                        <span   data-id="<%#((Userinfo_View)Container.DataItem).ID%>" class=" up glyphicon glyphicon-arrow-up" aria-hidden="true"></span>
                                        <span  data-id="<%#((Userinfo_View)Container.DataItem).ID%>" class=" down glyphicon glyphicon-arrow-down" aria-hidden="true"></span>

                                           </td>
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((Userinfo_View)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((Userinfo_View)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>                     
    ...
    <script>
        ...
        $('#deall').click(function () {
            ...
            layer.confirm('确定要删除全部信息？', {
                ...
                $.post('DeleteAll.aspx', { all: 1 }, function (result) {
                    if (result.isOk) {
        ...
        $('#set').click(function () {
            if ($("input[type='checkbox']:checked").length == 0) {
                layer.msg('请选择设置对象!', function () {
                    //关闭后的操作
                });
                return;
            }
            else {
                layer.confirm('确定要保存该用户？', {
                    btn: ['确定', '取消'] //按钮
                }, function () {
                    var arr = $('#theform').serializeArray();
                        $.post('ShenHe.aspx', arr, function (result) {
                            if (result.isOk)
                            {
