<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Manage.aspx.cs" Inherits="Plusbe2019_Admin_TableManageReserveMsg_Manage" %>
             ...
             <div class="ibox-content">
                        <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                            <div class="form-group">
                                <input type="text" placeholder="请输入预约人"  name="name" id="name"  runat="server" class="form-control">
                            </div>
                            <div class="form-group">
                                <input type="text" placeholder="请输入手机号"  name="phone" id="phone"  runat="server" class="form-control">
                            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((View_Reserve)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((View_Reserve)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                   
                                    <td><%# ((View_Reserve)Container.DataItem).ReserveName%></td>
                                    <td><%# ((View_Reserve)Container.DataItem).ReservePhone%></td>
                                    <td><%# ((View_Reserve)Container.DataItem).ReserveTime.ToShortDateString()%></td>
                                    <td><%# ((View_Reserve)Container.DataItem).Date%></td>
                                    <td><%# ((View_Reserve)Container.DataItem).ReserveCount%></td>
                                    <td><%# ((View_Reserve)Container.DataItem).AddTime%></td>
                                    <td><%# GetStates(((View_Reserve)Container.DataItem).States)%></td>
                                    <td class="text-c">
                                            <a href="#" class="btn btn-sm btn-info editmySelf" data-id="<%#((View_Reserve)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑 </a>
                                            <a href="#" class="btn btn-danger btn-sm deleteSelf" data-id="<%#((View_Reserve)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除 </a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
