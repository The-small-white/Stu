             ...
             <div class="ibox-content">
                        <form role="form" action="Manage.aspx" id="theform1" name="theform1" method="get" class="form-inline">
                            <div class="form-group">
                                <input type="text" placeholder="请输入用户名"  name="name" id="name"  runat="server" class="form-control">
                            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody class="text-c">
                                 <asp:Repeater ID="Repeater1" runat="server" DataSource="<%# m_tableManageList %>">
                                     <ItemTemplate>
                                <tr id="tr_<%# ((View_AdminUser)Container.DataItem).ID%>">
                                    <td> 
                                        <input type="checkbox" class="i-checks" name="checkshop" value="<%#((View_AdminUser)Container.DataItem).ID%>">
                                    </td>
                                    <td><%#Container.ItemIndex+1%></td>
                                    <td><%# ((View_AdminUser)Container.DataItem).Name%></td>
                                    <td><%# ((View_AdminUser)Container.DataItem).FullName%></td>
                                    <td><%# ((View_AdminUser)Container.DataItem).ExhibitionName%></td>
                                    <td><%# ((AdminMethod.EnumAdminLevel)((View_AdminUser)Container.DataItem).UserLevel).ToString()%></td>
                                    <td><%# GetStates(((View_AdminUser)Container.DataItem).States)%></td>
                                    <td><%# ((View_AdminUser)Container.DataItem).LastLoginTime.GetDateTimeFormats('f')[0].ToString()%></td>
                                    <td class="text-c">
                                            <a href="#"  class="btn btn-sm btn-info editmySelf"  data-id="<%#((View_AdminUser)Container.DataItem).ID%>"><i class="fa fa-pencil"></i> 编辑</a>
                                             <a href="#" class="btn btn-danger btn-sm deleteSelf"  data-id="<%#((View_AdminUser)Container.DataItem).ID%>"><i class="fa fa-remove"></i> 删除</a>
                                     </td>
                                </tr>
                                 </ItemTemplate>
                                 </asp:Repeater>
