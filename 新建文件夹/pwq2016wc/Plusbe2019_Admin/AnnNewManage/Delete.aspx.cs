...
public partial class Admin_ManageTableManage_Delete : AdminPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(this.Request["iD"]))
            {
                ...
                AddLog(iD + "");
                if (TableOperate<AnnNews>.Delete(iD) > 0)
                {
                    result.isOk = true;
            ...
            else if (!string.IsNullOrEmpty(this.Request["checkshop"]))
            {
                ...
                AddLog(checkshop);
                int id = TableOperate<AnnNews>.MultiDelete(checkshop);
                if (id > 0)
                {
                    result.isOk = true;
    ...
    protected void AddLog(string ids)
    {
        ...
        List<AnnNews> list = TableOperate<AnnNews>.Select(value, condition);
        if (list.Count > 0)
        {
            string deltitle = "";
            ...
            if (deltitle != "")
            {
                ...
                string logbrief = "管理员:【" + AdminMethod.AdminFullName + "】在" + DateTime.Now.GetDateTimeFormats('f')[0].ToString() + "删除了【" + deltitle + "】的公告";
