/************************************************************
  ...
  Description:    数据表City_Channel对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与City_Channel数据表对应对象
    /// </summary>
    public class City_Channel : IDateBuildTable
    {
        ...
        private bool isRed_initialized = false;
        ...
        public City_Channel(int iD, string name, int typeID, int depth, Boolean isRed, int parentID, int child, string path, int rootID, string namePath, int orderID, int seviceID, string pic, string brief)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "IsRed"))
            {
                if (dr["IsRed"] != null && dr["IsRed"] != DBNull.Value)
                {
                    this.IsRed = Convert.ToBoolean(dr["IsRed"]);
                }
            }
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || name_initialized || typeID_initialized || depth_initialized || isRed_initialized || parentID_initialized || child_initialized || path_initialized || rootID_initialized || namePath_initialized || orderID_initialized || seviceID_initialized || pic_initialized || brief_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            if (isRed_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "IsRed", SqlDbType.Bit);
                n_Parameter.Value = this.IsRed;
                n_Parameter.SourceColumn = "IsRed";
                parametersList.Add(n_Parameter);
            }
            ...
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Child", SqlDbType.Int);
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.isRed_initialized)
                {
                    contentText += ", [IsRed]=@" + headStr + "IsRed ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.isRed_initialized)
                {
                    conditionStr += " AND [IsRed]=@" + headStr + "IsRed ";
                }
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["isRed"] != null)
            {
                if (this.isRed_initialized)
                {
                    if(page.Request["isRed"] != "")
                    {
                        this.IsRed = Convert.ToBoolean(page.Request["isRed"]);
                    }
                    else
                    {
                        this.isRed_initialized = false;
                    }
                }
                else
                {
            ...
            if(page.Request["child"] != null)
            {
                if (this.child_initialized)
                {
                    ...
                    {
                        this.Child = Convert.ToInt32(page.Request["child"]);
                    }
                    else
                    {
