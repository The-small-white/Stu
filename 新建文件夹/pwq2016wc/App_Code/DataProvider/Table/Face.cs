/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/16 11:40:46
  Description:    数据表Face对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与Face数据表对应对象
    /// </summary>
    public class Face : IDateBuildTable
    {
        ...
        private bool feature_initialized = false;
        ...
        public Face(int iD, int userinfoID, string headImage, DateTime addTime, int exhibitionID, long orderID, Byte[] feature, int states)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "Feature"))
            {
                if (dr["Feature"] != null && dr["Feature"] != DBNull.Value)
                {
                    this.Feature = (byte[])(dr["Feature"]);
                }
            }
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || userinfoID_initialized || headImage_initialized || addTime_initialized || exhibitionID_initialized || orderID_initialized || feature_initialized || states_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Feature", SqlDbType.Binary);
                n_Parameter.Value = this.Feature;
                n_Parameter.SourceColumn = "Feature";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.feature_initialized)
                {
                    contentText += ", [Feature]=@" + headStr + "Feature ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.feature_initialized)
                {
                    conditionStr += " AND [Feature]=@" + headStr + "Feature ";
                }
