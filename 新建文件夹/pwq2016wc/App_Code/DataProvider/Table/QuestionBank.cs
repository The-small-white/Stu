/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/8/27 15:14:33
  Description:    数据表QuestionBank对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与QuestionBank数据表对应对象
    /// </summary>
    public class QuestionBank : IDateBuildTable
    {
        ...
        public QuestionBank(int iD, string title, int typeID, string files, string answer, int exhibitionID, long orderID, DateTime addTime, int addID, string brief, string fileSize, string trueList)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || title_initialized || typeID_initialized || files_initialized || answer_initialized || exhibitionID_initialized || orderID_initialized || addTime_initialized || addID_initialized || brief_initialized || fileSize_initialized || trueList_initialized)
                {
                    return false;
                }
                else
