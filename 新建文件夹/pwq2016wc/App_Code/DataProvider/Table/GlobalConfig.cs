/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/7/31 16:41:50
  Description:    数据表GlobalConfig对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与GlobalConfig数据表对应对象
    /// </summary>
    public class GlobalConfig : IDateBuildTable
    {
        ...
        private bool webName_initialized = false;
        
        private string m_brief;
        private bool brief_initialized = false;
         
        private int m_userIP;
        private bool userIP_initialized = false;
        ...
        public GlobalConfig(int iD, string webName, string brief, int userIP, int version, int addID, DateTime addTime)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "WebName"))
            {
                if (dr["WebName"] != null && dr["WebName"] != DBNull.Value)
                {
                    this.WebName = Convert.ToString(dr["WebName"]);
                }
            }
            ...
            if(CheckColumn(dr, "UserIP"))
            {
                if (dr["UserIP"] != null && dr["UserIP"] != DBNull.Value)
                {
                    this.UserIP = Convert.ToInt32(dr["UserIP"]);
                }
            }
        ...
        public string WebName
        {
            ...
            set
            {
                webName_initialized = true;
                this.m_webName = value;
            }
        }
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || webName_initialized || brief_initialized || userIP_initialized || version_initialized || addID_initialized || addTime_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            if (webName_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "WebName", SqlDbType.NVarChar);
                n_Parameter.Value = this.WebName;
                n_Parameter.SourceColumn = "WebName";
                parametersList.Add(n_Parameter);
            }
            ...
            if (userIP_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "UserIP", SqlDbType.Int);
                n_Parameter.Value = this.UserIP;
                n_Parameter.SourceColumn = "UserIP";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.webName_initialized)
                {
                    contentText += ", [WebName]=@" + headStr + "WebName ";
                }
                ...
                if (this.userIP_initialized)
                {
                    contentText += ", [UserIP]=@" + headStr + "UserIP ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.webName_initialized)
                {
                    conditionStr += " AND [WebName]=@" + headStr + "WebName ";
                }
                ...
                if (this.userIP_initialized)
                {
                    conditionStr += " AND [UserIP]=@" + headStr + "UserIP ";
                }
        ...
        public void AllInitialized()
        {
            ...
            this.webName_initialized = true;
            
            this.brief_initialized = true;
            
            this.userIP_initialized = true;
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["webName"] != null)
            {
                if (this.webName_initialized)
                {
                    if(page.Request["webName"] != "")
                    {
                        this.WebName = Convert.ToString(page.Request["webName"]);
                    }
                    else
                    {
                        this.webName_initialized = false;
                    }
                }
                else
                {
            ...
            if(page.Request["userIP"] != null)
            {
                if (this.userIP_initialized)
                {
                    if(page.Request["userIP"] != "")
                    {
                        this.UserIP = Convert.ToInt32(page.Request["userIP"]);
                    }
                    else
                    {
                        this.userIP_initialized = false;
                    }
                }
                else
                {
