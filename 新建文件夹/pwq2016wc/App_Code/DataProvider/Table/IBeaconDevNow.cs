/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/9/3 10:53:54
  Description:    数据表IBeaconDevNow对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// <summary>
    /// 与IBeaconDevNow数据表对应对象
    /// </summary>
    public class IBeaconDevNow : IDateBuildTable
    {
        ...
        public IBeaconDevNow()
        {
        }

        public IBeaconDevNow(int iD, int devID, int gateWayID, DateTime lastTime, double distance)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || devID_initialized || gateWayID_initialized || lastTime_initialized || distance_initialized)
                {
                    return false;
                }
                else
        ...
        {
            string tableName = "iBeaconDevNow";
            return tableName;
        }
