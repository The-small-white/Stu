/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/9/11 15:36:40
  Description:    数据表ModeChannel对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与ModeChannel数据表对应对象
    /// </summary>
    public class ModeChannel : IDateBuildTable
    {
        ...
        private int m_vsersion;
        private bool vsersion_initialized = false;
        ...
        public ModeChannel(int iD, int addID, DateTime addTime, string name, int typeID, int depth, int parentID, Boolean child, string path, int rootID, string namePath, int vsersion, string pic, string brief, int exhibitionID, int orderID)
        {
        ...
        public void FromIDataReader(IDataReader dr)
        {
            ...
            if(CheckColumn(dr, "Vsersion"))
            {
                if (dr["Vsersion"] != null && dr["Vsersion"] != DBNull.Value)
                {
                    this.Vsersion = Convert.ToInt32(dr["Vsersion"]);
                }
            }
        ...
        public int Vsersion
        {
            get
            {
                return this.m_vsersion;
            }
            set
            {
                vsersion_initialized = true;
                this.m_vsersion = value;
            }
        }
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || name_initialized || typeID_initialized || depth_initialized || parentID_initialized || child_initialized || path_initialized || rootID_initialized || namePath_initialized || vsersion_initialized || pic_initialized || brief_initialized || exhibitionID_initialized || orderID_initialized)
                {
                    return false;
                }
                else
        ...
        public SqlParameter[] GetSqlParameter(string headStr)
        {
            ...
            if (vsersion_initialized)
            {
                SqlParameter n_Parameter = new SqlParameter("@" + headStr + "Vsersion", SqlDbType.Int);
                n_Parameter.Value = this.Vsersion;
                n_Parameter.SourceColumn = "Vsersion";
                parametersList.Add(n_Parameter);
            }
        ...
        public string UpdateText(string headStr)
        {
            ...
            if(this.m_isAutoUpdate)
            {
                ...
                if (this.vsersion_initialized)
                {
                    contentText += ", [Vsersion]=@" + headStr + "Vsersion ";
                }
        ...
        public string ConditionText(string headStr)
        {
            ...
            if(this.m_isAutoConditon)
            {
                ...
                if (this.vsersion_initialized)
                {
                    conditionStr += " AND [Vsersion]=@" + headStr + "Vsersion ";
                }
        ...
        public void AllInitialized()
        {
            ...
            this.vsersion_initialized = true;
        ...
        public void AutoForm(System.Web.UI.Page page)
        {
            ...
            if(page.Request["vsersion"] != null)
            {
                if (this.vsersion_initialized)
                {
                    if(page.Request["vsersion"] != "")
                    {
                        this.Vsersion = Convert.ToInt32(page.Request["vsersion"]);
                    }
                    else
                    {
                        this.vsersion_initialized = false;
                    }
                }
                else
                {
