/************************************************************
  ...
  Copyright (c) 2008，
  Author:               Date: 2019/7/17 11:08:46
  Description:    数据表View_PCList对应的业务逻辑层
  Version:         1.1.0.0
...
namespace Dejun.DataProvider.Table
{
    ...
    /// 与View_PCList数据表对应对象
    /// </summary>
    public class View_PCList : IDateBuildTable
    {
        ...
        public View_PCList(int iD, int addID, DateTime addTime, string title, int version, int areaID, int state, string iP, string breif, DateTime lastTime, int exhibitionID, string areaName)
        {
        ...
        public bool IsNull
        {
            get
            {
                if (iD_initialized || addID_initialized || addTime_initialized || title_initialized || version_initialized || areaID_initialized || state_initialized || iP_initialized || breif_initialized || lastTime_initialized || exhibitionID_initialized || areaName_initialized)
                {
                    return false;
                }
                else
